...
func TestIntegrationGradingErrorKinds(t *testing.T) {
	...
	for _, tc := range []struct{ name, init, solution, user, kind string }{
		...
		{"student ambiguous column", "CREATE TABLE t (id INT)", "SELECT 1", "SELECT id FROM t a JOIN t b ON a.id = b.id", "student_query"},
		{"student invalid regexp", "", "SELECT 1", "SELECT REGEXP_LIKE('a', '[')", "student_query"},
		...
		{"reference ambiguous column", "CREATE TABLE t (id INT)", "SELECT id FROM t a JOIN t b ON a.id = b.id", "SELECT 1", "problem"},
		{"reference invalid regexp", "", "SELECT REGEXP_LIKE('a', '[')", "SELECT 1", "problem"},
		{"init syntax", "CREATE TABLE", "SELECT 1", "SELECT 1", "problem"},
		{"init no tables", "SELECT *", "SELECT 1", "SELECT 1", "problem"},
		{"init duplicate key", "CREATE TABLE t (id INT PRIMARY KEY); INSERT INTO t VALUES (1), (1)", "SELECT 1", "SELECT 1", "problem"},
