...
문제를 게시하거나 서비스를 업데이트한 뒤, 학생이 정상적으로 풀고 제출할 수 있는지 확인하는 절차입니다. SQL을 알고 관리자 화면을 사용할 수 있으면 따라갈 수 있습니다. 서버 코드·자동 테스트를 수정하는 방법은 [개발 가이드](../../../../docs/DEVELOPMENT.md)에 있습니다.
...
- **관리자 계정과 테스트용 학생 계정 A·B**: 관리자 계정은 접근 제한의 예외이므로 학생 동작은 학생 계정으로 확인해야 합니다. 관리자 **Users**에서 계정을 만들고 학번을 입력할 수 있습니다. 계정 준비와 첫 설정은 [운영 가이드](../../../../docs/TA_OPERATIONS.md)를 참고해 주세요.
...
시험 브라우저·단일 세션까지 점검하려면 [운영 가이드](../../../../docs/TA_OPERATIONS.md)의 설정표를 따라 주세요. 각각의 스위치와 저장 버튼을 확인해야 합니다.
...
장애를 의도적으로 만드는 검사, DB 비밀번호 변경, 새 설치의 DB 구조 확인은 개발·배포 담당자가 수행해야 합니다. [개발 가이드](../../../../docs/DEVELOPMENT.md)의 검증 항목을 요청하면 됩니다.
