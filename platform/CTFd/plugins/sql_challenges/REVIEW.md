...
1. **문제 내보내기.** 운영 또는 dev CTFd에서 SQL 문제 정의(지문, init, 정답, grading_policy)를 JSON으로 받습니다.
...
2. **검토 실행.** 일회용 MySQL 8.4과 이 checkout의 judge를 Docker Compose로 띄워 모든 문제를 실제로 채점하고, 끝나면 컨테이너와 volume을 지웁니다.
   ...
   출력의 첫 표는 문제별 상태·소요 시간·행 수이고, `== 검토 결과 ==` 아래에 확인이 필요한 문제와 이유가 나옵니다. 종료 코드가 0이 아니면 실행 실패 또는 채점 기준 미확정 문제가 있습니다.
...
3. **판정 읽기.** 검토 결과의 각 줄은 이렇게 처리합니다.
   ...
   | `policy_required` / 채점 기준 미확인 | 기존 문제의 공개 규약이 미확정 | 지문을 읽고 관리자 화면에서 정렬·표시 형식 기준을 저장한 뒤 다시 내보낸다. 원본 아카이브는 수정하지 않는다 |
...
judge 구현이 바뀌었을 때 이전 버전과 결과를 비교하려면 이전 judge를 같은 Compose 네트워크에 띄우고 `--baseline-url`을 넘깁니다. 결과 행이 다른 문제는 `value`, 정확히 같은 숫자의 표현만 다른 문제는 `numeric_format`으로 분류됩니다. 반올림 오차는 같은 값으로 취급하지 않습니다. 이 비교 보고서는 과거 점수를 재계산하거나 저장하지 않습니다.
...
- 정렬 동률은 judge가 MySQL로 검증합니다. 결과에 포함되지 않은 정렬 기준이나 LIMIT 경계의 행 선택은 지문·정답 설계를 검토해야 합니다.
