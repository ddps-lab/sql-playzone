...
시험 문제를 모두 만들었다면, 공개 전에 **채점기에 모범답안을 하나씩 제출해 보는 리허설**을 할 수 있습니다. 여기서 모범답안은 조교가 각 문제의 **Solution Query**에 저장한 정답 SQL입니다.

예를 들어 문제 20개를 준비했다면, 아래 도구가 20개 문제를 차례로 채점해 줍니다. 조교는 결과를 보고 오류가 있는 문제를 고친 뒤 다시 확인하면 됩니다.

```text
서비스에 저장된 문제와 모범답안 20개를 파일로 가져오기
    ↓
각 문제의 테이블·데이터를 임시 MySQL에 준비하기
    ↓
그 문제의 모범답안을 학생 답안으로 넣어 채점하기
    ↓
문제별 결과 확인: 정상 / SQL 오류 / 실행 지연 / 채점 기준 미설정
    ↓
조교가 해당 문제 수정 → 수정한 문제를 다시 가져와 채점
```

결과를 보고 조교가 할 일은 다음과 같습니다. 아래는 상황을 설명하기 위한 예시입니다.

| 문제 | 발견한 내용 | 조교가 할 일 |
|---|---|---|
| 1번 | 모범답안이 정상적으로 정답 처리됨 | 지문과 다른 풀이의 정답도 확인하기 |
| 2번 | 모범답안이 존재하지 않는 열을 조회함 | 테이블 정의와 모범답안의 열 이름을 맞추기 |
| 3번 | 실행 시간이 오래 걸림 | 데이터 규모·쿼리를 확인하고 실행 시간을 줄이기 |
| 4번 | 정렬 등 채점 기준을 아직 저장하지 않음 | 관리자 화면에서 지문에 맞는 기준을 저장하기 |

이 과정은 시험 문제를 한꺼번에 점검하거나 채점기 버전을 바꾼 뒤 기존 문제를 다시 확인할 때 유용합니다. 지문과 모범답안의 의미가 맞는지, 학생이 작성할 수 있는 다른 풀이도 정답으로 인정되는지는 동료 조교와 함께 검토해야 합니다.
...
## 3. 서비스에서 문제와 모범답안 가져오기

먼저 서비스에 저장한 문제를 `challenges.json` 파일로 가져옵니다. 다음 단계의 채점 도구가 이 파일을 읽어 각 문제를 준비할 수 있습니다.
...
가져온 파일로 채점을 시작하려면 같은 Bash 터미널·저장소 루트에서 다음을 실행해 주세요. 문제별 결과는 터미널에 표시되고 `report.json`에도 저장됩니다.
...
첫 실행에는 Docker 이미지 다운로드·빌드로 수 분이 걸릴 수 있습니다. 준비가 끝나면 도구는 파일에 들어 있는 문제마다 다음을 반복합니다.

1. 초기화 SQL을 실행해 테이블과 데이터를 준비합니다.
2. 모범답안 SQL을 **정답 SQL과 학생 SQL 양쪽에 넣어** 채점기에 보냅니다.
3. 채점기가 정답으로 인정하는지, 실행 오류가 있는지, 얼마나 오래 걸리는지 기록합니다.

문제 실행에는 이 컴퓨터에서 준비한 채점 서버(judge)와 임시 MySQL 8.4를 사용합니다. 작업이 끝나면 도구가 이번 검사용 컨테이너와 볼륨을 정리합니다.
...
| 표시 | 뜻 | 다음 조치 |
|---|---|---|
| `ok` | 학생 답안으로 넣은 모범답안이 정답 처리됨 | 지문·다른 풀이의 정답을 동료 조교와 확인해 주세요. |
...
종료 코드가 0이면 실행 오류 검사는 통과한 것입니다. **0행·느린 요청 같은 검토 알림은 종료 코드 0에서도 나올 수 있으므로 검토 결과를 함께 읽어야 합니다.**

수정할 문제가 있으면 서비스의 관리자 문제 화면에서 초기화 SQL·모범답안·채점 기준을 고쳐 주세요. 그다음 **3단계에서 문제 파일을 다시 받고 → 4단계에서 재채점**하면 됩니다. 관리자 화면에서 고친 내용은 파일을 다시 받아야 검사에 반영됩니다.
